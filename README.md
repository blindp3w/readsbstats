...
- **Military, interesting & anonymous aircraft**: flags surfaced as badges in the flight list and detail page, with counts on the statistics page. **Military** and **interesting** flags come from tar1090-db (OR-merged with `airplanes.live` overrides). **Anonymous** is computed at query time: it's set when the 24-bit Mode-S address falls outside every ICAO state-allocated block — typically military/OPSEC contacts, TIS-B / ADS-R rebroadcasts, or MLAT-synthetic identifiers. No DB column or backfill: editing the state-allocation table (`icao_ranges._RAW`) retroactively reclassifies every historical flight on the next query. Precedence (highest first) in badges, filters, and Telegram alerts: military > interesting > anonymous. A single aircraft surfaces under one kind, not all three. Dedicated **Gallery** page with card grid view showing photo, registration, type, country of origin, and flight count for every flagged aircraft detected
...
- **Telegram notifications**: optional bot notifications for first sighting of military / interesting / anonymous (non-ICAO hex) aircraft, emergency squawks (7500/7600/7700), watchlist hits, and configurable daily summary; military/interesting/anonymous/watchlist alerts include a **photo** when available (specific aircraft photo or type-level fallback with a caption note); interactive commands (`/summary`, `/status`, `/watchlist`, `/watch`, `/unwatch`, `/help`) via long polling; notification units (metric/imperial/aeronautical) are independently configurable; set `RSBS_TELEGRAM_PHOTOS=0` to disable photo enrichment; mute the anonymous-only kind with `RSBS_TELEGRAM_ANONYMOUS_ALERT=0`
...
- Web UI accessible via your existing nginx at `/stats/`
  ...
  - Flight list with search/filter by callsign, ICAO hex, registration, type, date, source, flag (`military` / `interesting` / `anonymous`), squawk code; merged route column (origin→destination)
  ...
  - Flagged aircraft gallery (`/gallery`): card grid of all detected military, interesting, and anonymous (non-ICAO hex) aircraft, with photo, registration, type, country of origin, flight count; filterable by flag type (all/military/interesting/anonymous — `all` shows the union), sortable by last seen, first seen, or flight count
...
│   ├── geo.py                  # Shared geometry (haversine, bearing, destination_point)
...
| GET | `/api/map/coverage` | Receiver coverage polygon (`window`: `24h`/`7d`/`30d`/`all`); 36-point Leaflet polygon, one vertex per 10° bearing bucket at the farthest position in that direction; per-window cache |
