...
// EXPERIMENTAL: actual OOOI (Out/Off/On/In) block times parsed from the
// airframe's ACARS bodies during this flight, with a ✓/✗ chip confirming the
// reported route against the scheduled origin/dest. Renders NOTHING when no OOOI
// body parsed and there's no `dsta` fallback — common on an H1-dominated feed.
...
export function OooiCard({ icao, firstSeen, lastSeen, scheduledOrigin, scheduledDest }: Props) {
  ...
  return (
    <Card data-testid="flight-oooi-card">
      <CardHeader>
        <CardTitle>OOOI block times</CardTitle>
      </CardHeader>
